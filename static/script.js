...
// When file selected → move to Generate
document.getElementById('audio-file-input')?.addEventListener('change', () => {
  setActiveStep('generate');
});

// When generate button enabled → highlight it
...
// When MIDI playback becomes available → Explore
const midiBtn = document.getElementById('play-midi-button');
if (midiBtn) {
  const observer = new MutationObserver(() => {
    if (!midiBtn.disabled) {
  ...
  observer.observe(midiBtn, { attributes: true });
}
...
// Unlock transpose when MIDI is ready
if (midiBtn) {
  const observer = new MutationObserver(() => {
    if (!midiBtn.disabled) {
      document
        .getElementById('transpose-container')
...
document.getElementById('play-midi-button')?.addEventListener('click', () => {
